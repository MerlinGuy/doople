...
wget -N $INSTALL_LOC/create_dopple_db.sql
wget -N $INSTALL_LOC/setup_dopple_db.sql
wget -N $INSTALL_LOC/create_core_tables.sql
wget -N $INSTALL_LOC/create_dopple_tables.sql

echo ""
echo "Configuring dopple db"
echo "... installing create_dopple_db.sql"
sudo -u postgres psql -f create_dopple_db.sql >&/dev/null
echo "... installing plpgsql"
sudo -u postgres createlang plpgsql dopple >&/dev/null
echo "... installing setup_dopple_db.sql"
sudo -u postgres psql -f setup_dopple_db.sql >&/dev/null
...
if [ "$chgPWD" = "x" ]; then
    sudo ./dpl_admin.sh -updatePwd -u admin -p $PWD1
fi
...
if [ "$RESP" = "y" ]; then
    ...
    sudo ./dpl_admin.sh -updateEmail -u admin -e $RESP
    echo ""
    HAS_ADMIN_EMAIL='y'
fi
...
if [ "$CREDS" = "y" ] && [ "$HAS_ADMIN_EMAIL" = "y" ]; then
    ...
    if [ "$RESP" = "y" ]; then
        sudo ./dpl_admin.sh -emailRsa -u admin
