...
public class GeofAdmin {
	...
	public boolean updatePassword(HashMap<String, String> mapArgs) {
		String[] reqd = "loginname,password".split(",");
		for (String key : reqd) {
			if (! mapArgs.containsKey(key)) {
				Logger.error("ERROR - updatePassword missing value for " + key);
				return false;
			}
		}
	...
	public boolean emailRsa(HashMap<String, String> mapArgs) {
		String[] reqd = {"loginname"};
		for (String key : reqd) {
			if (! mapArgs.containsKey(key)) {
				Logger.error("ERROR - sendRSA missing value for " + key);
				return false;
			}
		}
		boolean rtn =  RsaencryptionRequest.emailRsaByLogin(_dbi, mapArgs.get("loginname"));
		return rtn;
	}
	
	public boolean createRsa(HashMap<String, String> mapArgs) {
